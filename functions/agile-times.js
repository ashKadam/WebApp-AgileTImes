...
function addTimesheet(intent, session, callback) {
    ...
    if (projectSlot && typeof (projectSlot.value) !== 'undefined') {
        ...
        speechOutput = categorySlot && typeof (categorySlot.value) !== 'undefined' ? `Got it Ashwini. Adding new timesheet to ${project} project, ${category} category.` : `Got it Ashwini. Adding new timesheet to ${project} project.`;
...
function deleteTimeSheet(projectName, categoryName) {
    ...
    /*client.query(`
        mutation deleteTimesheet ($user: String!, $project: String!, $category: String!, $startTime: Int!, $endTime: Int!, $date: DateTime!) {
            deleteTimesheet(user: $user, project: $project, category: $category, startTime: $startTime, endTime: $endTime, date: $date ) {
                id
            }
        }`, variables, function (req, res) {
            if (res.status === 401) {
